...
function run(mode = process.env.INSPECTION_MODE || 'Repository') {
  ...
  if (fs.existsSync(homepagePath)) {
    ...
    if (!navbarFound || isSimulation) {
      findings.push({
        ...
        severity: 'P0',
        file: 'apps/web/src/app/page.tsx',
        evidence: "No Navbar or header components matches inside TSX file content",
        diagnostic: {
    ...
    if (!footerFound || isSimulation) {
      findings.push({
        ...
        severity: 'P1',
        file: 'apps/web/src/app/page.tsx',
        evidence: "No Footer or footer tag matches inside TSX file content",
        diagnostic: {
          ...
          dependencyImpact: {
            affectedFiles: ['apps/web/src/app/page.tsx'],
            affectedComponents: ['FooterComponent'],
            affectedRoutes: ['/'],
    ...
    if (!loginCtaFound || isSimulation) {
      findings.push({
        ...
        severity: 'P0',
        file: 'apps/web/src/app/page.tsx',
        evidence: "No Link referencing /login or custom SignIn click elements",
        diagnostic: {
          ...
          dependencyImpact: {
            affectedFiles: ['apps/web/src/app/page.tsx'],
            affectedComponents: ['NavbarComponent', 'HeroComponent'],
            affectedRoutes: ['/'],
    ...
    findings.push({
      ...
      severity: 'P0',
      file: 'apps/web/src/app/page.tsx',
      evidence: "Physical file check returned non-existence at path apps/web/src/app/page.tsx",
      diagnostic: {
        ...
        dependencyImpact: {
          affectedFiles: ['apps/web/src/app/page.tsx'],
          affectedComponents: ['HomepageController'],
          affectedRoutes: ['/'],
      ...
      for (const pattern of config.forbiddenPatterns) {
        if (line.includes(pattern) || (isSimulation && pattern === 'href="#"')) {
          findings.push({
            ...
            evidence: `Found placeholder: ${line.trim()}`,
            diagnostic: {
              lineNumber: i + 1,
              ...
              dependencyImpact: {
                ...
                affectedComponents: ['SidebarComponent'],
  ...
  if (fs.existsSync(homepagePath)) {
    ...
    if (!contents.includes('min-h-screen') || !contents.includes('mx-auto')) {
      findings.push({
        ...
        severity: 'P1',
        file: 'apps/web/src/app/page.tsx',
        evidence: "Missing min-h-screen container wrapper class",
        diagnostic: {
          ...
          dependencyImpact: {
            affectedFiles: ['apps/web/src/app/page.tsx'],
            affectedComponents: ['MainContentWrapper'],
            affectedRoutes: ['/'],
  ...
  const trustScore = metrics.getSentinelTrustScore('UI Sentinel', findings);
  ...
  const reportPath = utils.getReportPath('ui', 'report.json');
