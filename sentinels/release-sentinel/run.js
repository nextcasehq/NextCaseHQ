...
function run(mode = process.env.INSPECTION_MODE || 'Repository') {
  ...
  const outputDir = path.join(rootDir, 'reports', 'bevs');
  ...
  const reportPath = utils.getReportPath('release', 'release-report.json');
  fs.writeFileSync(reportPath, JSON.stringify(reportsStack, null, 2));

  // Read previous execution trends from history
  const historyPath = utils.getReportPath('release', 'history.json');
  let history = [];
