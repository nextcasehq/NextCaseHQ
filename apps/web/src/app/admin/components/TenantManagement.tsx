  ...
  return (
    <div className="space-y-6 bg-white border border-[#F4EEE0] p-6 rounded-2xl shadow-sm">
      <div>
        ...
        <p className="text-sm text-[#726B58] font-serif italic mt-0.5">Authorize, isolate, or migrate multi-tenant databases with Row-Level Security rules.</p>
      </div>
          ...
          <tbody>
            {tenants.map((t, idx) => (
              <tr key={idx} className="border-b border-[#FBF8F1] hover:bg-[#FBF8F1]/50 transition-colors font-semibold">
                <td className="py-4 px-4 font-mono text-xs text-[#6F5624]">{t.id}</td>
                <td className="py-4 px-4 text-[#111111]">{t.name}</td>
                <td className="py-4 px-4 text-[#6F5624]">{t.region}</td>
                <td className="py-4 px-4 text-[#6F5624]">{t.type}</td>
