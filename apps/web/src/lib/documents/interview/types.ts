...
/**
 * The Legal Interview configuration layer's shared data shape. Every
 * guided interview — regardless of court vertical (Supreme Court, High
 * Courts, District Courts, Magistrate Courts, Other Courts & Tribunals) or
 * document type — is one `InterviewConfig` value: a generic interview-
 * engine schema plus a description of how its answers map onto a
 * template's placeholder tokens. Adding a new interview never requires new
 * application code — only a new schema file and one registry entry (see
 * registry.ts) — because every court's requirements are expressed as
 * configuration/data here, never as court-specific branches in the engine
 * or draft-generation code.
 *
 * This module is the one place allowed to know both "interview engine"
 * (lib/interview-engine) and "document generation" (fill-template.ts,
 * templates.ts) concepts at once — the generic engine itself never
...
/**
 * A repeatable group's (interview-engine `group` field) answers rendered
 * into one HTML fragment — e.g. one `<p>` per petitioner — and substituted
 * for a single token, e.g. `[PETITIONERS_BLOCK]`.
 */
export interface ListFieldConfig {
  questionName: string;
  renderItem: (item: Record<string, unknown>, index: number) => string;
  /** Rendered when the repeatable group has zero entries. */
  empty?: string;
}
 ...
 * the engine, fill-template, or registry today (none of them read this
...
export interface InterviewConfig {
  ...
  templateId: string;
  title: string;
  /** Renamed from `surveyJson` — this interview's generic interview-engine
   *  schema (pages/fields/groups/conditions), no longer an opaque
   *  SurveyJS-shaped bag now that NextCaseHQ owns the engine consuming it. */
  schema: EngineSchema;
