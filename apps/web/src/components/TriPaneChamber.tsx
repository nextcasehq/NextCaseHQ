...
export const TriPaneChamber = () => {
  ...
  const [activePanel, setActivePanel] = useState(1); // For mobile dockets (0: Ledger, 1: Dialogue, 2: Draft)
  ...
  // Interactive filters
  const [timelineFilter, setTimelineFilter] = useState('ALL'); // Date filter slider
  const [selectedEntity, setSelectedEntity] = useState<string | null>(null); // Graph interaction
          ...
          {rightTab === 'draft' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full font-mono font-bold uppercase">Draft v1</span>
              </div>
              <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-md min-h-[450px] flex flex-col justify-between">
                <div className="border-b border-neutral-100 pb-4 mb-4 text-center">
                  <h3 className="font-extrabold text-xs tracking-widest text-neutral-800 uppercase">IN THE HIGH COURT OF DELHI</h3>
                  <p className="font-serif text-[10px] italic text-neutral-500 mt-1">Writ Petition No. 132/2026</p>
                </div>
                <div className="flex-1 font-serif text-[13px] text-neutral-700 space-y-3 leading-relaxed">
                  <p className="font-bold">IN THE MATTER OF:</p>
                  <div className="flex justify-between font-semibold">
                    <span>NextCaseHQ Technologies Inc.</span>
                    <span>...Petitioner</span>
                  </div>
                  <p className="text-center py-1 text-xs text-neutral-400">VERSUS</p>
                  <div className="flex justify-between font-semibold">
                    <span>Union of India & Ors.</span>
                    <span>...Respondents</span>
                  </div>
                  <div className="h-px bg-neutral-100 my-4" />
                  <p className="text-justify font-sans text-xs italic bg-neutral-50 p-4 rounded-xl border border-neutral-100 text-neutral-500">
                    Draft locked to active session RLS context. Citations are verified using immutable ledger checks.
                  </p>
                </div>
              </div>
            </div>
