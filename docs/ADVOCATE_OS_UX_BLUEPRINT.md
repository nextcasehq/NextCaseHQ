...
3. **Second 20–60:** The advocate taps either action once. If they tap Prepare Document, they see a practice-area/document-type picker (§9) — no Matter required yet, since `DocumentEnvelope.case_id`/`matter_id` are already nullable in the schema. If they tap Record Court Note, they see the Court Note screen with a lightweight Matter/Proceeding picker; if none exists yet, it offers **"+ Quick-create a Matter"** inline, reusing the existing Matter-creation form (already built on `/matters`) rather than a new capability — a Court Note still always belongs to a real Proceeding (`CourtNote.case_id` is `NOT NULL` by design and this blueprint does not propose changing that), it is just created in the same flow instead of as a separate, earlier setup step.
...
**This is not a demotion of Court Note relative to Prepare Document or the two-entry-point model.** A center FAB is, by convention and by this design's own treatment (larger, elevated, gold-accented, always in the same reach-zone regardless of scroll position), the single most prominent element on the bar — a *stronger* visual claim than an equally-weighted fifth tab would make, not a weaker one. Court Note is additionally repeated as the single largest element on Home (§7, item 2, alongside Prepare Document) and inline on every Matter/Proceeding screen (already built). Nothing in this recommendation reduces Court Note's status as one of the product's two primary entry points (§2) — it changes *where the tap lands from*, not *how important the action is*.
...
- **Matter/Proceeding entry point is currently case-only.** Today's only path in is `/cases/[id]/court-note` — an advocate must already have navigated to a specific Proceeding. §4 and §6 both assume a lightweight, standalone Court Note entry (FAB from anywhere) that starts with a Matter/Proceeding picker, with an inline "+ Quick-create a Matter" option (reusing the existing Matter-creation form) when none exists yet; neither the picker nor the inline quick-create currently exists and together are the one concrete gap between the current implementation and the recommended navigation model. This is additive UI work only — no backend or schema change (`POST /api/cases/[id]/court-notes` already takes any valid `case_id`; `CourtNote.case_id` remains `NOT NULL`, unchanged — a Court Note is still always tied to a real Proceeding, just created in the same flow instead of as a separate earlier step).
...
## 22a. Trust and Legal Control — Explicit Reaffirmation

Called out on its own, not only implied by §9/§14/§15, because it is non-negotiable rather than a design preference: nothing in this blueprint implies or should ever be built to imply that NextCaseHQ files a document automatically, sends a communication without explicit confirmation, silently modifies finalized legal text, treats an AI or voice-dictated suggestion as a verified fact rather than advocate-reviewed content, or guarantees a legal outcome. Every consequential screen this blueprint proposes (Court Note save, Prepare Document draft, any future AI-assisted step) keeps the advocate's review and explicit confirmation as the last step before anything is treated as final — matching Milestone 1's own Court Note implementation exactly, and matching `enforceEntitlement()`'s existing pattern of being a real, unconditionally-called checkpoint regardless of what it currently allows. The advocate remains responsible and in control; the product remembers, organizes, and prepares — it does not decide or act on the advocate's behalf.
