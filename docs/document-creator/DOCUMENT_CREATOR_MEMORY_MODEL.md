...
`DOCUMENT_CREATOR_EXECUTION_KERNEL.md` §1 ("Authoritative-State Architecture") is this Memory Model's Cold Memory / Hot Memory split, restated at the kernel's own internal-design level, with one addition worth naming explicitly here: the kernel may maintain a lightweight **in-memory State Store** — a cache of "which job is this worker currently holding" — for performance. That State Store is Hot Memory like everything else in this document's Hot Memory list; it is explicitly barred (kernel §1) from being authoritative for draft content, generation-job status, document versions, AI Credit reservations or debits, worker leases, retries, indexing status, audit history, tenant ownership, or advocate confirmation — the same list Rule 1 above already requires be reconstructable from PostgreSQL and object storage. Nothing in the kernel document expands what Hot Memory is allowed to be authoritative for; it only names the specific cache a worker implementation is likely to hold, so a future implementer doesn't mistake "a cache exists" for "the cache is now a system of record."
