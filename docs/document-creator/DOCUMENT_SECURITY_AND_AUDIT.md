...
The Audit Events list above is not a separately-maintained checklist an implementer must remember to satisfy — it is structurally enforced by `DOCUMENT_CREATOR_EXECUTION_KERNEL.md` §3.3's command-handler contract, step 6: "produce an audit event." Every command in the kernel's catalogue (§3.1) maps to at least one entry in this document's event list (e.g., `AutosaveDraftCommand` → the Autosave event; `ReserveAiCreditsCommand`/`ReleaseAiCreditReservationCommand` → AI Credit events; `ConfirmAdvocateReviewCommand` → Advocate confirmation). A command handler that does not emit its required audit event is, by the kernel's own contract, an incorrectly-implemented handler — this is a structural guarantee, not a convention to remember. The kernel's §13 ("Security Boundaries") restates the server-resolved-identity and no-client-trust rules from this document's "Identity and Trust" section at the kernel's own component level (every layer from the API route through the Policy Engine to the Worker Runtime).
