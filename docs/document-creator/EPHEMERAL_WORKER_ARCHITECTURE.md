...
`DOCUMENT_CREATOR_EXECUTION_KERNEL.md` §8 defines a model-independent `WorkerRuntime` interface abstracting exactly the mechanics this document specifies (claim, lease renewal, context retrieval, provider invocation, streamed output, cancellation, timeout, retry classification, cleanup, heartbeat, graceful shutdown) — this document is the behavioral specification; that interface is its future TypeScript shape. The kernel document's §9 restates this lifecycle as 20 steps against its own finer-grained state machine (splitting this document's step 6 "load only relevant Matter Register context" into a distinct `CONTEXT_LOADING` state, and its own steps 11–12 "create permanent DocumentVersion" into a distinct `VERSION_CREATED` state) — the two describe one lifecycle, not two different ones. Where an implementer needs the finer-grained state names for `DocumentGenerationJob.status` persistence, the kernel document's §4 table is authoritative; this document's 17 numbered steps remain the correct plain-language description of worker behavior at every granularity.
